-- Wallet ↔ Asset association table
CREATE TABLE IF NOT EXISTS strategy.tb_wallet_asset (
...
-- Wallet strategy: category target allocation percentages
CREATE TABLE IF NOT EXISTS strategy.tb_wallet_strategy (
    id          BIGSERIAL PRIMARY KEY,
    wallet_id   INTEGER       NOT NULL REFERENCES strategy.tb_wallet(id) ON DELETE CASCADE,
