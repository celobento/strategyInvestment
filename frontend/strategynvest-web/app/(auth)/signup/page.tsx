  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(body || 'Failed to create account.')
      }
    ...
    >
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm mt-1" style={{ color: '#4a5e4d' }}>Create your account</p>
        </div>

        {/* Card */}
        <div className="rounded-2xl border shadow-sm p-8 space-y-5" style={{ backgroundColor: '#e0decb', borderColor: '#b8b5a0' }}>
          <div className="space-y-1">
            <h2 className="text-xl font-semibold" style={{ color: '#001c1f' }}>Sign up</h2>
            <p className="text-sm" style={{ color: '#4a5e4d' }}>Fill in the details to get started</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label htmlFor="email" className="text-sm font-medium" style={{ color: '#001c1f' }}>
                Email <span className="text-xs font-normal" style={{ color: '#6a7e6d' }}>(optional)</span>
              </label>
              <input
                id="email"
                type="email"
            ...
            <div className="space-y-1.5">
              <label htmlFor="confirmPassword" className="text-sm font-medium" style={{ color: '#001c1f' }}>Confirm Password</label>
              <input
                id="confirmPassword"
                type="password"
                required
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center">
              <span className="px-2 text-xs uppercase" style={{ backgroundColor: '#e0decb', color: '#6a7e6d' }}>or continue with</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 h-11 rounded-lg border text-sm font-medium transition hover:bg-white/60 disabled:opacity-50"
          ...
          <p className="text-xs text-center" style={{ color: '#4a5e4d' }}>
            ...
            <Link href="/signin" className="font-medium hover:underline" style={{ color: '#6ba513' }}>Sign in</Link>
          </p>
        </div>
